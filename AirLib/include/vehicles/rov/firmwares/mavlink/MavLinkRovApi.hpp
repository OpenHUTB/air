...
#include "vehicles/rov/firmwares/mavlink/MavLinkOutputMappings.hpp"
...
namespace airlib
{
    ...
    class MavLinkRovApi : public RovApiBase
    {
    public: //methods
        virtual ~MavLinkRovApi()
        {
            closeAllConnection();
        ...
        virtual void sendTelemetry(float last_interval = -1) override
        {
            ...
            // This method is called at high frequence from RovPawnSimApi::updateRendering.
        ...
        void start_telemtry_thread()
        {
            ...
            this->telemetry_thread_ = std::thread(&MavLinkRovApi::telemtry_thread, this);
        }
    ...
    protected: //methods
        virtual void connect()
        {
            if (!connecting_) {
                ...
                this->connect_thread_ = std::thread(&MavLinkRovApi::connect_thread, this);
            }
        }
    ...
    private: //methods
        ...
        {
            if (connection_ == nullptr)
                throw std::domain_error("MavLinkRovApi requires connection object to be set before createProxy call");
        ...
        void processMavMessages(const mavlinkcom::MavLinkMessage& msg)
        {
            if (msg.msgid == HeartbeatMessage.msgid) {
                ...
                throw std::invalid_argument("HilControlsMessage not supported in MavLinkRovApi");
            }
                ...
                for (size_t i = 0; i < actuator_mapping_.size(); ++i) {
                    if (isarmed && (actuator_mapping_[i] != -1)) {
    ...
    private: //variables
        ...
        std::shared_ptr<RovApiBase> mav_vehicle_control_;
        ...
        //additional variables required for RovApiBase implementation
