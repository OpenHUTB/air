...
namespace airlib
{
    ...
    void RovApiBase::resetImplementation()
    {
        cancelLastTask();
        SingleTaskCall lock(this); //cancel previous tasks
    }

    bool RovApiBase::takeoff(float timeout_sec)
    {
    ...
    bool RovApiBase::land(float timeout_sec)
    {
        SingleTaskCall lock(this);
    ...
    bool RovApiBase::goHome(float timeout_sec)
    {
        SingleTaskCall lock(this);
    ...
    bool RovApiBase::moveByVelocityBodyFrame(float vx, float vy, float vz, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
    {
        SingleTaskCall lock(this);
    ...
    bool RovApiBase::moveByVelocityZBodyFrame(float vx, float vy, float z, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
    {
        SingleTaskCall lock(this);
    ...
    bool RovApiBase::moveByMotorPWMs(const vector<float>& pwm_values, float duration)
    {
        SingleTaskCall lock(this);
    ...
    bool RovApiBase::moveByRollPitchYawZ(float roll, float pitch, float yaw, float z, float duration)
    {
        SingleTaskCall lock(this);
    ...
    bool RovApiBase::moveByRollPitchYawThrottle(float roll, float pitch, float yaw, float throttle, float duration)
    {
        SingleTaskCall lock(this);
    ...
    bool RovApiBase::moveByRollPitchYawrateThrottle(float roll, float pitch, float yaw_rate, float throttle, float duration)
    {
        SingleTaskCall lock(this);
    ...
    bool RovApiBase::moveByRollPitchYawrateZ(float roll, float pitch, float yaw_rate, float z, float duration)
    {
        SingleTaskCall lock(this);
    ...
    bool RovApiBase::moveByAngleRatesZ(float roll_rate, float pitch_rate, float yaw_rate, float z, float duration)
    {
        SingleTaskCall lock(this);
    ...
    bool RovApiBase::moveByAngleRatesThrottle(float roll_rate, float pitch_rate, float yaw_rate, float throttle, float duration)
    {
        SingleTaskCall lock(this);
    ...
    bool RovApiBase::moveByVelocity(float vx, float vy, float vz, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
    {
        SingleTaskCall lock(this);
    ...
    bool RovApiBase::moveByVelocityZ(float vx, float vy, float z, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
    {
        SingleTaskCall lock(this);
                                ...
                                float lookahead, float adaptive_lookahead)
    {
        ...
        if (velocity > getRovApiParams().breaking_vel) {
            breaking_dist = Utils::clip(velocity * getRovApiParams().vel_to_breaking_dist,
                                        getRovApiParams().min_breaking_dist,
                                        getRovApiParams().max_breaking_dist);
        }
        ...
        while (!waiter.isTimeout() && (next_path_loc.seg_index < path_segs.size() - 1 || goal_dist > 0)) { //current position is approximately at the last end point
            ...
            if (seg_velocity > getRovApiParams().min_vel_for_breaking && path_length_remaining <= breaking_dist) {
                seg_velocity = getRovApiParams().breaking_vel;
    ...
    bool RovApiBase::moveByManual(float vx_max, float vy_max, float z_min, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
    {
        SingleTaskCall lock(this);
        ...
        do {
            ...
            if (rc_data.is_valid && (rc_data.timestamp == 0 || age <= kMaxMessageAge)) { //if rc message timestamp is not set OR is not too old
                ...
                catch (const RovApiBase::UnsafeMoveException& ex) {
    ...
    bool RovApiBase::rotateToYaw(float yaw, float timeout_sec, float margin)
    {
        SingleTaskCall lock(this);
    ...
    bool RovApiBase::rotateByYawRate(float yaw_rate, float duration)
    {
        SingleTaskCall lock(this);
    ...
    void RovApiBase::setAngleLevelControllerGains(const vector<float>& kp, const vector<float>& ki, const vector<float>& kd)
    {
        uint8_t controller_type = 2;
        setControllerGains(controller_type, kp, ki, kd);
    }

    void RovApiBase::setAngleRateControllerGains(const vector<float>& kp, const vector<float>& ki, const vector<float>& kd)
    {
        uint8_t controller_type = 3;
        setControllerGains(controller_type, kp, ki, kd);
    }

    void RovApiBase::setVelocityControllerGains(const vector<float>& kp, const vector<float>& ki, const vector<float>& kd)
    {
        uint8_t controller_type = 4;
        setControllerGains(controller_type, kp, ki, kd);
    }

    void RovApiBase::setPositionControllerGains(const vector<float>& kp, const vector<float>& ki, const vector<float>& kd)
    {
        uint8_t controller_type = 5;
        setControllerGains(controller_type, kp, ki, kd);
    }

    bool RovApiBase::hover()
    {
        SingleTaskCall lock(this);
    ...
    void RovApiBase::moveByRC(const RCData& rc_data)
    {
        unused(rc_data);
        //by default we say that this command is not supported
        throw VehicleCommandNotImplementedException("moveByRC API is not implemented for this Rov");
    }

    void RovApiBase::moveByVelocityInternal(float vx, float vy, float vz, const YawMode& yaw_mode)
    {
        if (safetyCheckVelocity(Vector3r(vx, vy, vz)))
            commandVelocity(vx, vy, vz, yaw_mode);
    }

    void RovApiBase::moveByVelocityZInternal(float vx, float vy, float z, const YawMode& yaw_mode)
    {
        if (safetyCheckVelocityZ(vx, vy, z))
            commandVelocityZ(vx, vy, z, yaw_mode);
    }

    void RovApiBase::moveToPositionInternal(const Vector3r& dest, const YawMode& yaw_mode)
    {
        if (safetyCheckDestination(dest))
            commandPosition(dest.x(), dest.y(), dest.z(), yaw_mode);
    }

    void RovApiBase::moveByRollPitchYawZInternal(float roll, float pitch, float yaw, float z)
    {
        if (safetyCheckVelocity(getVelocity()))
            commandRollPitchYawZ(roll, pitch, yaw, z);
    }

    void RovApiBase::moveByRollPitchYawThrottleInternal(float roll, float pitch, float yaw, float throttle)
    {
        if (safetyCheckVelocity(getVelocity()))
            commandRollPitchYawThrottle(roll, pitch, yaw, throttle);
    }

    void RovApiBase::moveByRollPitchYawrateThrottleInternal(float roll, float pitch, float yaw_rate, float throttle)
    {
        if (safetyCheckVelocity(getVelocity()))
            commandRollPitchYawrateThrottle(roll, pitch, yaw_rate, throttle);
    }

    void RovApiBase::moveByRollPitchYawrateZInternal(float roll, float pitch, float yaw_rate, float z)
    {
        if (safetyCheckVelocity(getVelocity()))
            commandRollPitchYawrateZ(roll, pitch, yaw_rate, z);
    }

    void RovApiBase::moveByAngleRatesZInternal(float roll_rate, float pitch_rate, float yaw_rate, float z)
    {
        if (safetyCheckVelocity(getVelocity()))
            commandAngleRatesZ(roll_rate, pitch_rate, yaw_rate, z);
    }

    void RovApiBase::moveByAngleRatesThrottleInternal(float roll_rate, float pitch_rate, float yaw_rate, float throttle)
    {
        if (safetyCheckVelocity(getVelocity()))
    ...
    Waiter RovApiBase::waitForFunction(WaitFunction function, float timeout_sec)
    {
        Waiter waiter(getCommandPeriod(), timeout_sec, getCancelToken());
        if (timeout_sec <= 0)
            return waiter;
    ...
    bool RovApiBase::waitForZ(float timeout_sec, float z, float margin)
    {
        float cur_z = 100000;
        return waitForFunction([&]() {
                   cur_z = getPosition().z();
    ...
    void RovApiBase::setSafetyEval(const shared_ptr<SafetyEval> safety_eval_ptr)
    {
        SingleCall lock(this);
        safety_eval_ptr_ = safety_eval_ptr;
    }

    RCData RovApiBase::estimateRCTrims(float trimduration, float minCountForTrim, float maxTrim)
    {
    ...
    void RovApiBase::moveToPathPosition(const Vector3r& dest, float velocity, DrivetrainType drivetrain, /* pass by value */ YawMode yaw_mode, float last_z)
    {
        unused(last_z);
        //validate dest
        if (dest.hasNaN())
    ...
    bool RovApiBase::emergencyManeuverIfUnsafe(const SafetyEval::EvalResult& result)
    {
        if (!result.is_safe) {
    ...
    bool RovApiBase::safetyCheckVelocity(const Vector3r& velocity)
    {
    ...
    bool RovApiBase::safetyCheckVelocityZ(float vx, float vy, float z)
    {
        if (safety_eval_ptr_ == nullptr) //safety checks disabled
            return true;
    ...
    bool RovApiBase::safetyCheckDestination(const Vector3r& dest_pos)
    {
        if (safety_eval_ptr_ == nullptr) //safety checks disabled
            return true;
    ...
    void RovApiBase::adjustYaw(const Vector3r& heading, DrivetrainType drivetrain, YawMode& yaw_mode)
    {
    ...
    void RovApiBase::adjustYaw(float x, float y, DrivetrainType drivetrain, YawMode& yaw_mode)
    {
        adjustYaw(Vector3r(x, y, 0), drivetrain, yaw_mode);
    }

    bool RovApiBase::isYawWithinMargin(float yaw_target, float margin) const
    {
    ...
    float RovApiBase::getObsAvoidanceVelocity(float risk_dist, float max_obs_avoidance_vel) const
    {
        unused(risk_dist);
